$(document).ready(() => {
  ...
  // 클래스명이 wrap인 태그의 모든 후손들 중 span 태그의 스타일 변경
  ...
  $('.wrap1>h2').nextAll().css('background-color', 'aqua');
  ...
  $('.wrap1>h2').prev().css('background-color', 'orange');
  
  // h2 태그의 이전에 있는 모든 형제 요소에 스타일 적용
  $('.wrap1>h2').prevAll().css('background-color', 'aqua');
  
  // h2 태그 이전부터 p태그 이후까지 모든 형제 요소에 스타일 적용
