...
btn4.addEventListener('click', () => {
    ...
    // Date 객체를 생성하는 방법
    let date1 = new Date();
    let date2 = new Date(1636591311768); //밀리초단위 기준 날로부터
    let date3 = new Date('2021/11/11/09:28:00');
    let date4 = new Date(2021, 10, 11, 9, 31, 59); // 월은 0부터 시작( 년, 월, 일 )
    ...
    area.innerHTML += `${date1.getMonth() + 1} <br>`;
    area.innerHTML += `${date1.getDate()} <br>`;
    area.innerHTML += `${date1.getDay()} <br>`; // 요일관련 메소드 -> 0: 일요일, ... 6: 토요일
